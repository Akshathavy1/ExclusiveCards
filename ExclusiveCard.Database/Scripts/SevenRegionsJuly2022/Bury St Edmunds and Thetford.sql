...
SELECT @Name ='Bury St Edmunds & Thetford Rewards'
SELECT @DisplayName = 'Bury St Edmunds & Thetford Rewards'
SELECT @URL = 'https://invalid.exclusiverewards.co.uk' --<< LIVE
--SELECT @URL = 'https://localhost:44325/'
SELECT @Slug = 'exclusive'
SELECT @CompanyNumber ='11616720'
SELECT @CSSFile = 'exclusive.css'
SELECT @Logo = 'logo.svg' 
SELECT @NewsletterLogo = 'logo_dark_newsletter.png'
...
Declare @registrationCode Varchar(max) = 'SPRXXXXXXX' --'RegCode'
...
BEGIN
  ...
  VALUES
  (@Name + ' featured', 'Featured local offer list',0,1,1,'Show All',0,@NewWhiteLabelId,2),
  (@Name + ' main', 'Main body local offer list',0,1,1,'Show All',0,@NewWhiteLabelId,1)
END
...
DECLARE @CampaignId int
  ...
  BEGIN
    ...
    VALUES
    (
       @NewsletterId, @NewWhiteLabelId, NULL, REPLACE(REPLACE(@Name, ' ', ''), '&','') + '_Campaign', 1068141, 0
