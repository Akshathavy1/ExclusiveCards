...
DECLARE @DiamondNeeded bit = 0 --<< 1 = add diamond plan, 0 = don't add diamond plan
DECLARE @DiamondCardPrice decimal(18,2) = 9.99
...
BEGIN
	...
	SELECT @CharityName = 'Southport FC'
	SELECT @CharityUrl = 'https://southportfc.net/'
END
ELSE
BEGIN
...
SELECT @Name ='Southport FC Rewards'
SELECT @DisplayName = 'Southport FC Rewards'
SELECT @URL = 'https://southportfc.exclusiverewards.co.uk' --<< LIVE
SELECT @Slug = 'southport-fc'
SELECT @CompanyNumber ='11616720'
SELECT @CSSFile = 'southport-fc.css'
...
Declare @registrationCode Varchar(max) = 'SFCSSR01' --'RegCode'
Declare @DiamondRegistrationCode Varchar(max) = 'SFCSDR01' --'RegCode'
...
BEGIN
	...
	VALUES
	 ('GB', @FaceBook,'https://www.facebook.com/SouthportFootballClub/', @NewWhiteLabelId),
	 ('GB', @Twitter,'https://twitter.com/southport_fc', @NewWhiteLabelId),
	 ('GB', @Instagram,'https://www.instagram.com/southportfootballclub/', @NewWhiteLabelId),
	 ('GB', @YouTube,'https://www.youtube.com/c/SouthportFCTV/videos', @NewWhiteLabelId)
END
  ...
  END
	...
	BEGIN
		...
		INSERT [Exclusive].[MembershipPlan] ([PartnerId], [MembershipPlanTypeId], [Duration] ,[NumberOfCards]  ,[ValidFrom]  ,[ValidTo]   ,[CustomerCardPrice]  ,[PartnerCardPrice]
			,[CustomerCashbackPercentage]  ,[DeductionPercentage]  ,[IsDeleted]  ,[CurrencyCode]   ,[Description]   ,[IsActive]  ,[MembershipLevelId]   ,[PaidByEmployer]   ,[MinimumValue]
			,[PaymentFee]   ,[CardProviderId], [BenefactorPercentage],[WhitelabelId] )
		VALUES (1, 4, 365, 100000000, GetDate(), GetDate() + 36500, @DiamondCardPrice, 0, @DiamondCustomerCashbackPercentage, @DiamondDeductionPercentage, 0, 'GBP', @CardProvider + @DiamondPlanText, 1, 2, 0, 10, 1, @CardProviderId, @DiamondBeneficiaryPercentage, @NewWhiteLabelId)
