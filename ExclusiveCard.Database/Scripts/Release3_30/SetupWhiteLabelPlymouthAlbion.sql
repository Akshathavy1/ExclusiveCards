...
SELECT @Name ='Plymouth Albion Rewards'
SELECT @DisplayName = 'Plymouth Albion'
SELECT @URL = 'https://plymouthalbion.exclusiverewards.co.uk' --<< LIVE
SELECT @Slug = 'plymouth-albion'
SELECT @CompanyNumber ='11616720'
SELECT @CSSFile = 'plymouth-albion.css'
...
SELECT @Address = 'Plymouth Albion RFC, Brickfields Recreation Ground, 25 Damerel Close, Plymouth, PL1 4NE'
SELECT @CardName ='Exclusive Rewards'
SELECT @Privicy = '/Account/PrivacyPolicy?country=GB'
SELECT @Terms = '/Account/Terms?country=GB'
SELECT @CharityName = 'Plymouth Albion Sports club'
SELECT @CharityUrl = 'http://www.plymouthalbion.com/'
...
BEGIN
	...
	VALUES
	 ('GB', @Instagram,'https://www.instagram.com/officialplymouthalbion/', @NewWhiteLabelId),
	 ('GB', @FaceBook,'https://www.facebook.com/AlbionRFC/', @NewWhiteLabelId),
	 ('GB', @Twitter,'https://twitter.com/AlbionRFC', @NewWhiteLabelId),
     ('GB', @YouTube,'https://www.youtube.com/channel/UCpK8VXFPnTjF0ZlhZj_ivvQ', @NewWhiteLabelId)
END
