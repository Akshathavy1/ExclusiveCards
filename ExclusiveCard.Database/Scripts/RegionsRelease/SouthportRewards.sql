...
--## Southport is a Regions site !!! ##
DECLARE @IsRegional int = 1 --<<Set this to 1 if this is a regions site with local offers

SELECT @Name ='Southport Rewards'
SELECT @DisplayName = 'Southport Rewards'
SELECT @URL = 'https://southport.exclusiverewards.co.uk' --<< LIVE
SELECT @Slug = 'southport'
SELECT @CompanyNumber ='11616720'
SELECT @CSSFile = 'southport.css'
...
Declare @registrationCode Varchar(max) = 'SOUTHP1'--'SOL2021' --'RegCode'
DECLARE @DiamondRegistrationCode Varchar(max) = 'SOUTHPD1'

-- Edit these values for beneficiary plans
Declare @customerCashbackPercentage INT = 81--56 -- 81
Declare @deductionPercentage INT = 19
Declare @beneficiaryPercentage INT = 0--25 --0
-- End editing
