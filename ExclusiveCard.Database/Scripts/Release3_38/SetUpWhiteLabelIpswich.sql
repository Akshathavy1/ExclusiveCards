...
SELECT @Name ='Ipswich Town Rewards'
SELECT @DisplayName = 'Ipswich Town FC'
SELECT @URL = 'https://Ipswichtownfc.exclusiverewards.co.uk' --<< LIVE
SELECT @Slug = 'ipswich-town'
SELECT @CompanyNumber ='11616720'
SELECT @CSSFile = 'ipswich-town.css'
...
SELECT @Address = 'Ipswich Town Football Club, Portman Road, Ipswich, Suffolk, IP1 2DA'
SELECT @CardName ='Exclusive Rewards'
SELECT @Privicy = '/Account/PrivacyPolicy?country=GB'
SELECT @Terms = '/Account/Terms?country=GB'
SELECT @CharityName = 'Ipswich Town Football Club'
SELECT @CharityUrl = 'https://www.itfc.co.uk'
...
BEGIN
	...
	VALUES
	 ('GB', @Instagram,'https://www.instagram.com/ipswichtown/', @NewWhiteLabelId),
	 ('GB', @FaceBook,'https://www.facebook.com/officialitfc', @NewWhiteLabelId),
	 ('GB', @Twitter,'https://twitter.com/IpswichTown', @NewWhiteLabelId),
     ('GB', @YouTube,'https://www.youtube.com/user/officialitfc', @NewWhiteLabelId)
END
...
Declare @cardProvider Varchar(max) = 'Ipswich Town Rewards' --'Card Provider'
Declare @registrationCode Varchar(max) = 'ITS2021' --'RegCode'
