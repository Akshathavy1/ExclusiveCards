...
SELECT @Name ='Exceed Rewards'
SELECT @DisplayName = 'Exceed Rewards'
SELECT @URL = 'https://exceed.exclusiverewards.co.uk' --<< LIVE
SELECT @Slug = 'exceed-rewards'
SELECT @CompanyNumber ='11616720'
SELECT @CSSFile = 'exceed-rewards.css'
...
BEGIN
	...
	SELECT @DefaultRegion = Id FROM [CMS].[WhiteLabelSettings] WHERE [Name] = 'Kingston Upon Thames Rewards'
END
